
export type Alignment = 'start' | 'center' | 'end' | 'baseline' | 'stretch';
export type BorderToken = 'all' | 'top' | 'bottom' | 'left' | 'right' | 'dashed' | 'none';
export type FontWeightToken = 'light' | 'normal' | 'medium' | 'semibold' | 'bold';
export type Justification = 'start' | 'center' | 'end' | 'between' | 'around';
export type ReactLevelToken = 'success' | 'warning' | 'danger' | 'info';
export type SizeToken = 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl';
export type SurfaceToken = 'base' | 'subtle' | 'sunken' | 'transparent';
export type RoundedToken = 'none' | 'sm' | 'md' | 'lg' | 'xl' | '2xl' | 'full';
export type SpacingToken = 'none' | 'xs' | 'sm' | 'md' | 'lg' | 'xl';
export type TagToken = 'p' | 'span' | 'strong' | 'div' | 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'label' | 'legend' | 'li';
export type VariantToken = 'primary' | 'secondary' | 'tertiary' | 'ghost' | 'outline';

export type ThemeToken = ReactLevelToken | SurfaceToken | VariantToken;
